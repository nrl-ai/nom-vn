...
phục dấu (3 cách chạy: quy tắc / HF seq2seq / LLM), tách từ + câu,
chuẩn hoá NFC + nhận diện tiếng Việt, bỏ dấu, sinh nhiễu (7 kiểu cấu
hình, đầu ra ổn định).
...
| **OCR (form / cấp trang VLM)** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/handwriting.md) | [`5CD-AI/Vintern-1B-v3_5`](https://huggingface.co/5CD-AI/Vintern-1B-v3_5) (zero-shot, qua `nom.ocr.handwriting`) | MIT | ~1.8 GB | **CER 0.47 %** sạch · **0.37 %** nhiễu trên [`nrl-ai/vn-synthetic-ocr`](https://huggingface.co/datasets/nrl-ai/vn-synthetic-ocr) (n=20 mỗi điều kiện) | Truyền cả trang, không cắt từng dòng — VLM ảo trên line crop hẹp (qwen2.5vl 33 % CER trên chữ in clean). n nhỏ; bench trên 200+ ảnh trước khi publish con số headline. |
| **STT (giọng nói tiếng Việt)** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/stt.md) | [`vinai/PhoWhisper-large`](https://huggingface.co/vinai/PhoWhisper-large) (qua `nom.stt`) | BSD-3 | ~3 GB | n=3 smoke trên `doof-ferb/Speech-MASSIVE_vie`: 15.2 % WER (ngang `whisper-large-v3` trên tập này) | VinAI claim VIVOS 4.67 % / VLSP T1 13.75 % WER — chưa tái lập ở đây; phải bench trên ViMD 3-vùng (Bắc/Trung/Nam) trước khi claim dialect coverage. |
| **Tóm tắt (tin tức VN)** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/summarize.md) | [`VietAI/vit5-large-vietnews-summarization`](https://huggingface.co/VietAI/vit5-large-vietnews-summarization) (mô hình có sẵn, gọi qua `nom.summarize`) | MIT | ~3.3 GB | ROUGE-1 63,4 trên vietnews (số do tác giả công bố). n=28 wiki_vi nội bộ: **7/28 mẫu thêm số liệu mới không có trong nguồn** (ví dụ năm "2025", chỉ số GDP "6,8 % – 7,0 %"). | Đừng triển khai cho tóm tắt pháp lý / tài chính mà không kiểm chứng từng số — số mới xuất hiện là tín hiệu bịa thật. Mô hình mã hoá-giải mã giới hạn 1024 token; tài liệu dài cần Qwen3-8B + LoRA (Đợt 3, chưa ship). |
| **Phân loại văn phong** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/register.md) | [`nrl-ai/vn-register-phobert-base`](https://huggingface.co/nrl-ai/vn-register-phobert-base) (PhoBERT-base + đầu phân loại 4 lớp, của chúng tôi) | MIT | ~540 MB | **macro F1 0,900** trên test n=1234 (trang trọng 0,91 / kinh doanh 0,91 / hội thoại 0,92 / văn học 0,87) | Bộ phân loại 4 văn phong tiếng Việt đầu tiên có giấy phép công cộng (lấp khoảng trống nghiên cứu mà khảo cứu của chúng tôi đã chỉ ra). Bản quy tắc dự phòng cũng ship trong OSS cho định tuyến không cần học máy. |
| **NER (regex VN)** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/ner.md) | `nom.nlp.ner.RegexNERModel` + mở rộng pháp lý (`nom.nlp.ner_legal`, ship trong OSS) | Apache 2.0 | <1 KB | Ngưỡng cơ sở dùng biểu thức chính quy — bộ chuẩn (`PER / ORG / LOC / DATE / MONEY`); bộ pháp lý thêm `LAW_REF / ID_VN / PHONE_VN` cho công văn VN. Bộ kiểm thử xác minh các mẫu; chưa có F1 trên kho VN gold-labeled. | Tinh chỉnh PhoBERT với lớp đầu ra tự thiết kế cho `LAW_REF + CONTRACT_PARTY` cần ~70-90 giờ chú thích (Đợt 3, chưa bắt đầu). |
...
- *Quan tâm độ chính xác thực tế (không chỉ tổng hợp)?* Đọc [bench ngoài-phân-phối OOD](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/spell-correction.md) — 175 mẫu lỗi tiếng Việt thật trên 6 văn phong, kèm khoảng tin cậy 95 % bằng bootstrap. Tóm tắt (v0.2.29): tổng hợp 98,32 % nhiễu nhẹ trung bình; OOD tổng hợp **89,54 %**, so với `Toshiiiii1` 87,29 % và `bmd1905` 57,86 %.
...
- 🤗 [`nrl-ai/vn-diacritic-vit5-base`](https://huggingface.co/nrl-ai/vn-diacritic-vit5-base) — bản tinh chỉnh ViT5-base khôi phục dấu, cân bằng thể loại
...
- **[docs/benchmark.md](https://github.com/nrl-ai/nom-vn/blob/main/docs/benchmark.md)** — số đo theo từng mô-đun (chứng cứ sau mỗi dòng "Stack khuyến nghị" ở trên)
...
- **[docs/training_plan_2026q2.md](https://github.com/nrl-ai/nom-vn/blob/main/docs/training_plan_2026q2.md)** — khi nào nên tinh chỉnh thay vì dùng mô hình có sẵn, theo từng mô-đun, kèm ước tính chi phí
...
- **[CONTRIBUTING.md](https://github.com/nrl-ai/nom-vn/blob/main/CONTRIBUTING.md)** — cài đặt môi trường phát triển, quy tắc PR
