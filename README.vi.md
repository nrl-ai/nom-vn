...
| **Phân loại văn phong** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/register.md) | `nom.classify.LexiconRegisterClassifier` (zero-ML, ship trong OSS) | Apache 2.0 | <1 KB | Heuristic — pass test trên các từ-mốc của chính nó (tautology). PhoBERT fine-tune script ở `training/register/` đã sẵn nhưng GPU run đang chờ. | Dùng cho routing rẻ tiền hôm nay; checkpoint PhoBERT sẽ publish thành `nrl-ai/vn-register-phobert-base` khi chạy xong. |
