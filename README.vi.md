...
| **Tóm tắt (tin tức VN)** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/summarize.md) | [`VietAI/vit5-large-vietnews-summarization`](https://huggingface.co/VietAI/vit5-large-vietnews-summarization) (off-the-shelf, qua `nom.summarize`) | MIT | ~3.3 GB | Upstream ROUGE-1 63.4 vietnews. n=10 wiki_vi nội bộ: **1/10 thêm năm "2025" không có trong input**; trước đó 1 mẫu 234 ký tự bịa số GDP "6,8 % – 7,0 %". | Đừng ship cho tóm tắt pháp lý / tài chính mà không audit hallucination — số mới xuất hiện là tín hiệu thật. Encoder-decoder cap 1024 token; tài liệu dài cần Qwen3-8B + LoRA (Tier 3, chưa ship). |
| **Phân loại văn phong** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/register.md) | [`nrl-ai/vn-register-phobert-base`](https://huggingface.co/nrl-ai/vn-register-phobert-base) (PhoBERT-base + đầu 4-class, của chúng tôi) | MIT | ~540 MB | **macro F1 0,900** trên test n=1234 (formal 0,91 / business 0,91 / hội thoại 0,92 / văn học 0,87) | Bộ phân loại 4-register VN đầu tiên có giấy phép công cộng (đáp ứng research gap chỉ ra trong khảo cứu của chúng tôi). Bản quy tắc fallback cũng ship trong OSS cho định tuyến không cần học máy. |
| **NER (regex VN)** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/ner.md) | `nom.nlp.ner.RegexNERModel` + mở rộng pháp lý (`nom.nlp.ner_legal`, ship trong OSS) | Apache 2.0 | <1 KB | Regex baseline — preset chuẩn (`PER / ORG / LOC / DATE / MONEY`); preset pháp lý thêm `LAW_REF / ID_VN / PHONE_VN` cho công văn VN. Test verify pattern; chưa có F1 gold-standard. | PhoBERT fine-tune với head custom `LAW_REF + CONTRACT_PARTY` cần ~70-90 giờ chú thích (Tier 3, chưa bắt đầu). |
