...
| --- | --- | --- | --- | --- | --- |
| **`madlad400-3b-mt`** ⭐ | EN → VN | **40.92** | **23.39** | **56.83 %** | 260 ms |
| `m2m100_418M` | EN → VN | 35.73 | 16.33 | 53.06 % | 870 ms |
| `m2m100_418M` | VN → EN | 38.63 | 20.64 | (không đo) | 696 ms |

**Khuyến nghị:** dùng `google/madlad400-3b-mt` cho chiều EN → VN —
hơn `m2m100_418M` 5 điểm chrF, 7 điểm BLEU, và nhanh hơn 3.3 lần
trên GPU. Mô hình lớn hơn (3 GB) nhưng chất lượng đáng giá.

`m2m100_418M` vẫn là lựa chọn tốt khi chỉ có CPU, hoặc cho chiều
VN → EN (chưa đo MADLAD chiều này).

> **Lưu ý kỹ thuật:** `transformers >= 5` hỏng nặng cho MADLAD —
> khoá embedding đầu vào / đầu ra không được "tied" đúng, mô hình
