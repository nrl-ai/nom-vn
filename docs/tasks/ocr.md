...
| 🖨️ **Văn bản in nhiễu nhẹ** (quét hơi mờ, blur 1-2px) | **Tesseract `vie`** | **0.70 %** | 80 ms | dùng VietOCR nếu cần xử lý chữ thường + hoa lẫn lộn (latin_PP-OCRv5 nhỉnh hơn trên một số font) |
...
| ✍️ **Chữ viết tay** (cá nhân, học sinh, ghi chép) | **VietOCR** | **31.82 %** | 246 ms | PaddleOCR PP-OCRv5 thứ hai (59.4 %) nhưng vẫn kém VietOCR ~28 pp |
| 📋 **Form / hoá đơn / ID có layout** | **VLM** (Qwen2.5VL, Gemma3-Vision) | TBD | 1.4-2 s | Tesseract / VietOCR cho các trường text thuần khi đã trích được crop |
| 📷 **Ảnh điện thoại nhiễu mạnh** (perspective, lighting) | **VLM** ở mức tài liệu hoặc preprocess + VietOCR | TBD | 1.4-2 s | VLM hoán đảo tốt với OCR rule-based khi ảnh có ngữ cảnh |
| 📜 **Sách cổ / vintage** (1850-2000) | TBD — chưa đo trên VieBookRead | TBD | TBD | Future work; VieBookRead corpus afl-3.0 |
...
| PaddleOCR PP-OCRv5 (latin_mobile_rec) | Apache 2.0 | — | — | — | 59.43 % | 1110 ms |
| EasyOCR | Apache 2.0 | — | — | — | 71.52 % | 60 ms |
| TrOCR base-handwritten (English) | MIT | — | — | — | 75.89 % | 248 ms |
| `qwen2.5vl:7b` (VLM) | Apache 2.0 (model) | 31.07 % | not benched | not benched | not benched | 1.4 s |
...
- **VietOCR là engine đúng cho chữ viết tay tiếng Việt.** -37.5 pp
  ...
  PaddleOCR PP-OCRv5 đứng thứ 2 nhưng vẫn kém VietOCR ~28 pp + chậm
  4-5 lần.
...
- **PaddleOCR PP-OCRv5 cho tiếng Việt = `latin_PP-OCRv5_mobile_rec`.**
  Không có model VN-specific trong PP-OCRv5 family; latin script
  recognizer phủ bảng chữ tiếng Việt nhưng không tối ưu hoá riêng.
...
nào trong landscape gần ngang. PaddleOCR PP-OCRv5 đứng thứ 2 ở
59.4 % CER (gấp 2 lần lỗi của VietOCR + chậm 5 lần).
