...
| PaddleOCR PP-OCRv5 (latin_mobile_rec) | Apache 2.0 | 11.63 % | 13.09 % | 73.40 % | 59.43 % | 1110 ms |
| RapidOCR (ONNX, PaddleOCR port) | Apache 2.0 | 63.97 % | 77.83 % | 100.00 % | 97.20 % | 90 ms |
...
- **PaddleOCR PP-OCRv5 đứng thứ 3 trên *mọi* register tiếng Việt** —
  re-test 2026-05-01 sau khi thử default pipeline, doc-prep skipped,
  standalone `TextRecognition`, và `latin_PP-OCRv5_server_rec`. Lý do
  cấu trúc: **PP-OCRv5 không ship recognizer VN-specific**; `lang='vi'`
  chỉ load `latin_PP-OCRv5_mobile_rec` — model Latin script generic
  nhận diện được bảng chữ cái nhưng strip dấu thanh (acute / grave /
  hook / tilde / dot below) ở mức cao trên cả printed clean.
