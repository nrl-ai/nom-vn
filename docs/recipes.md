...
OOD tổng hợp 71.15 % — kém spell-correction-base 8.47 pp trên văn
bản thực tế hỗn hợp, nhưng nhanh + nhẹ hơn khi đầu vào chỉ thiếu
dấu. Xem [`/tasks/diacritic-restoration`](/tasks/diacritic-restoration)
cho phân tích đánh đổi đầy đủ.

#### Suy luận theo lô để tăng thông lượng (nhanh hơn 7,6× trên 3080)

Với pipeline cần thông lượng cao (chục nghìn câu), dùng
``predict_batch`` thay vì lặp ``predict``:
...
```python
# Sản xuất — PhoBERT-base + đầu phân loại 4 lớp (đã tinh chỉnh sẵn)
...
Tinh chỉnh checkpoint riêng:

```bash
python training/register/train.py \
    --output-dir checkpoints/register-phobert-base \
    --epochs 4
# Mục tiêu macro-F1 ≥ 0,85 trên 20 % giữ lại của kho UDHR + VNTC +
# Tatoeba + Wikisource. Xem training/register/README.md.
```

**Quy tắc ngón cái:** demo / chỉ CPU / không cần độ chính xác cao
→ phương án từ vựng. Định tuyến sản xuất trong pipeline server →
PhoBERT.

### Tách từ tiếng Việt

Hai cách chạy, chọn theo tốc độ vs F1:

```python
# Ưu tiên tốc độ — Python thuần, không phụ thuộc, F1 76 % trên UD_Vietnamese-VTB
...
```python
# Ưu tiên chất lượng — mô hình CRF, F1 95.7 %
import underthesea
...
**Quy tắc ngón cái:** lập chỉ mục RAG / BM25 / dọn nhẹ → `nom.text`.
NER / phân tích cú pháp phụ thuộc / tác vụ ngôn ngữ → `underthesea`.

### Trích xuất thực thể VN — bộ chuẩn + bộ pháp lý

Quy tắc, không cần GPU, đầu ra ổn định:
...
- **Cần loại thực thể tự định nghĩa** (ID nhân viên, mã hợp đồng
  nội bộ) → `RegexNERModel(extra_patterns=[("EMPLOYEE_ID", r"NV\d{6}"), ...])`.

NER dựa trên PhoBERT đầy đủ (PER chính xác, F1 ≥ 90 %) cần tinh
chỉnh trên VLSP-NER + chú thích tay cho `LAW_REF` / `CONTRACT_PARTY`
— đợt sau, chưa làm.
...
- [`docs/training_plan_2026q2.md`](training_plan_2026q2.md) — khi nào nên tinh chỉnh thay vì dùng mô hình có sẵn
