...
| **Summarisation (VN news)** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/summarize.md) | [`VietAI/vit5-large-vietnews-summarization`](https://huggingface.co/VietAI/vit5-large-vietnews-summarization) (off-the-shelf, ours via `nom.summarize`) | MIT | ~3.3 GB | Upstream ROUGE-1 63.4 vietnews. Internal n=10 wiki_vi: **1/10 added a year not in input ("2025")**; an earlier 234-char sample fabricated GDP "6,8 % – 7,0 %" figures. | Don't ship for legal / finance summarisation without a hallucination audit — flagged numeric novelty is a real signal. Encoder-decoder caps input at 1024 tokens; for longer documents use Qwen3-8B + LoRA (Tier 3, not yet shipped). |
| **Register classifier** [→](https://github.com/nrl-ai/nom-vn/blob/main/docs/tasks/register.md) | `nom.classify.LexiconRegisterClassifier` (zero-ML, ships in OSS) | Apache 2.0 | <1 KB | Heuristic — passes its own marker tests (tautology). PhoBERT fine-tune script under `training/register/` is code-complete but the GPU run is pending. | Use for cheap upstream routing today; the published-on-HF PhoBERT head will land as `nrl-ai/vn-register-phobert-base` once the run completes. |
